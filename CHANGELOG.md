...
- Speech-to-text providers gain an **extra request body** field: a JSON object merged into the transcription request, for parameters HGA does not expose as its own fields. It is available on both the OpenAI and Local provider types and is merged last, so it can also override a field HGA sets.
