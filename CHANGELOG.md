...
- **Night + day away-motion rules on the same sensors no longer double-alert** — a household keeping its night rule and approving the day-agnostic sibling would have received two pushes (and doubled audit rows) for every night motion event. The evaluator now emits only the night rule's finding when both fire on overlapping sensors.
