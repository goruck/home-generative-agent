...
- **Exclusion entries are validated against match-everything mistakes** — with glob support, a stray `"*"` or `"*.*"` entry would silently match every entity and could disable the entire detection engine (findings *and* triggers) with nothing in the UI to show for it. Every entry must now contain a dot and at least one literal character: the settings form rejects offending entries (and over-long ones beyond 256 characters) with a validation error, and hand-edited `.storage` entries failing the rule are ignored at startup with a log warning — fail-closed, so the affected entity resumes alerting rather than going dark. The engine also warns when an exclusion *type key* contains glob characters, since type keys are matched exactly and such a key would be silently inert. Pre-existing stored entries that fail the new validation stop applying after upgrade.
