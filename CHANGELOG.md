...
- **Reloading no longer disconnects features that are still using the connection.** Conversation, speech-to-text, and image entities borrow the integration's database pool and OpenAI connection. Reloads closed both while those entities were still live, which could surface as a conversation failing mid-reply or losing its last memory write. Entities are now shut down first, and everything they borrow is released afterwards.
