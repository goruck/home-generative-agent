...
- `network_guest_client_present`: the Home Assistant security audit reports a device on your guest Wi-Fi that you never trusted when it is connected while nobody is home (medium) or at night (low) ([network security plan](docs/network-security-plan.md), step 7). Guests on the guest network while someone is home in daytime are what the network is for, so nothing is reported then. Devices you parked on the guest network on purpose were trusted when the device inventory was established, and a visitor's phone stops being reported once you tap **Trust device** on the push, so what is left is a device that was announced when it joined, was never vouched for, and keeps coming back. A device's first day belongs to `network_unknown_device_joined`, so one arrival never pushes twice; at most one alert a day. Needs a router integration that says which clients are on the guest network (the [eero](https://github.com/schmittx/home-assistant-eero) integration today); when some connected clients come from a source that does not say, the check is listed as not run instead of passing. `network_unknown_device_joined` now also says when a new device joined over the guest Wi-Fi.
