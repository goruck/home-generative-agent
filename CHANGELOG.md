...
- **Explanations no longer claim a near-zero thermometer "is not drawing power."** The LLM explanation guidance for near-zero readings is now conditioned on instantaneous power sensors; cumulative energy counters (which reset) are never described as "off", and other sensor types get neutral "reading is near zero" phrasing.
