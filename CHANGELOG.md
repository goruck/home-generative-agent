...
- **Services that run stored configuration are gated**: `python_script` (whose sandbox permits `hass.services.call`), `shell_command`, `rest_command`, and `button.press` / `input_button.press` (a template button's press field is a full script).
