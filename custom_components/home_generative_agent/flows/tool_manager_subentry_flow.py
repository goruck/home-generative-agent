...
class ToolManagerSubentryFlow(ConfigSubentryFlow):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            self._payload[CONF_TOOL_RETRIEVAL_LIMIT] = int(user_input[CONF_TOOL_RETRIEVAL_LIMIT])
            self._payload[CONF_TOOL_RELEVANCE_THRESHOLD] = float(user_input[CONF_TOOL_RELEVANCE_THRESHOLD])
            self._payload[CONF_INSTRUCTION_RETRIEVAL_LIMIT] = int(user_input[CONF_INSTRUCTION_RETRIEVAL_LIMIT])
            self._payload[CONF_INSTRUCTION_RELEVANCE_THRESHOLD] = float(user_input[CONF_INSTRUCTION_RELEVANCE_THRESHOLD])
        ...
        providers = [llm.LLM_API_ASSIST] + [api.id for api in apis if api.id != llm.LLM_API_ASSIST]
        providers.append("langchain_internal") # Add custom internal tool provider
        
        for p in providers:
            actions.append(SelectOptionDict(label=f"Configure Provider: {p}", value=f"edit_provider_{p}"))
            
        # Add instruction options
        actions.append(SelectOptionDict(label="Add New Prompt and Tag", value="add_instruction"))
        instructions = self._payload.get(CONF_INSTRUCTIONS_CONFIG, {})
        for i_name in sorted(instructions.keys()):
            actions.append(SelectOptionDict(label=f"Configure Instruction: {i_name}", value=f"edit_instruction_{i_name}"))
        ...
        for api in apis:
            try:
                ...
                pass
        active_tools.update(["get_and_analyze_camera_image", "get_camera_last_events", "upsert_memory", "get_entity_history", "confirm_sensitive_action", "alarm_control", "resolve_entity_ids", "write_yaml_file", "get_available_tools", "add_automation"])
        
        for t in sorted(active_tools):
            actions.append(SelectOptionDict(label=f"Configure Tool: {t}", value=f"edit_tool_{t}"))
    ...
    async def async_step_provider_editor(self, user_input: dict[str, Any] | None = None) -> SubentryFlowResult:
        if user_input is not None:
        ...
        current_cfg = self._payload.get(CONF_TOOL_PROVIDERS, {}).get(self._provider_to_edit, {})
        schema = vol.Schema({
            vol.Required("enabled", default=current_cfg.get("enabled", True)): BooleanSelector(),
            vol.Optional("prompt", default=current_cfg.get("prompt", ""), description={"suggested_value": current_cfg.get("prompt", "")}): TemplateSelector(),
            vol.Optional("tags", default=current_cfg.get("tags", ""), description={"suggested_value": current_cfg.get("tags", "")}): TextSelector()
        })
    ...
    async def async_step_tool_editor(self, user_input: dict[str, Any] | None = None) -> SubentryFlowResult:
        if user_input is not None:
        ...
        current_cfg = self._payload.get(CONF_TOOLS_CONFIG, {}).get(self._tool_to_edit, {})
        schema = vol.Schema({
            vol.Required("enabled", default=current_cfg.get("enabled", True)): BooleanSelector(),
            vol.Optional("prompt", default=current_cfg.get("prompt", ""), description={"suggested_value": current_cfg.get("prompt", "")}): TemplateSelector(),
            vol.Optional("tags", default=current_cfg.get("tags", ""), description={"suggested_value": current_cfg.get("tags", "")}): TextSelector()
    ...
    async def async_step_instruction_name(self, user_input: dict[str, Any] | None = None) -> SubentryFlowResult:
        if user_input is not None:
        ...
        return self.async_show_form(
            ...
            description_placeholders={
                "instruction_name_title": "Instruction Name"
            }
        )

    async def async_step_instruction_editor(self, user_input: dict[str, Any] | None = None) -> SubentryFlowResult:
        if user_input is not None:
        ...
        current_cfg = self._payload.get(CONF_INSTRUCTIONS_CONFIG, {}).get(self._instruction_to_edit, {})
        schema = vol.Schema({
            vol.Required("enabled", default=current_cfg.get("enabled", False)): BooleanSelector(),
            vol.Optional("prompt", default=current_cfg.get("prompt", ""), description={"suggested_value": current_cfg.get("prompt", "")}): TemplateSelector(),
            vol.Optional("tags", default=current_cfg.get("tags", ""), description={"suggested_value": current_cfg.get("tags", "")}): TextSelector(),
