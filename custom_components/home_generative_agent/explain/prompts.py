...
SYSTEM_PROMPT = (
    ...
    "'failed'.\n"
    "In baseline_deviation and time_of_day_anomaly evidence, `baseline_value` and "
    "`expected_value` are historical averages (what is normal), NOT the current "
    "sensor reading. `current_value` is the live reading that triggered the alert. "
    "Never describe `baseline_value` or `expected_value` as 'current state', "
    "'current reading', or 'present value'.\n"
    "If evidence contains current_value=0 (or a very small number below 2) and "
    "deviation_direction='below', AND the evidence device_class is power (or "
    "unit_of_measurement is an instantaneous power unit like W or kW), the "
    "appliance is OFF or not drawing power. Never say it is 'running', "
    "'operating', or 'active' — say it is 'off' or 'not drawing power'. For "
    "cumulative energy counters (device_class energy, kWh) a near-zero value "
    "only means little energy was recorded in the period — counters reset; "
    "never claim the appliance is off. For any other sensor type "
    "(temperature, humidity, illuminance, ...), never mention power — say "
    "the reading is near zero / well below its usual level.\n"
