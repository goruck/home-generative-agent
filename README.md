...
| **Sentinel anomaly detection** | Deterministic rules watch for security and safety issues (unlocked locks, open entries, unknown people — the unknown-person rules require the face-service) and alert your phone. A built-in [Home Assistant security audit](docs/sentinel.md#home-assistant--network-security) also flags new admin accounts and access tokens, old long-lived tokens, locks exposed to voice assistants, failed logins, exposed add-on ports, public webhooks, offline security devices, unconfigured devices found on the LAN, newly paired Zigbee/Z-Wave/Bluetooth/Matter devices, Z-Wave locks included without S2 security, an open Zigbee2MQTT join window, pending coordinator firmware, UPnP left on at the router (plus public-IP changes and newly opened UPnP port mappings when the core UPnP/IGD integration is set up), and, with any router integration whose trackers report `source_type: router` (FRITZ!Box, UniFi, eero, …), devices that join your network for the first time, with a Trust button when one device joined, plus (with eero) an untrusted device on the guest Wi-Fi while nobody is home, a guest Wi-Fi left on unused, WPA3 off, threat blocking off, dynamic DNS on, and a device moving far more data than usual for the hour (see the [eero setup notes](docs/sentinel.md#home-assistant--network-security) for the three eero settings that matter) — none of the rest needs a router integration. Ask "is my home secure?" and the agent runs the same checks on demand and tells you which ones it could not run. An option keeps the audit's details out of the chat model entirely: the model gets counts and fixed titles, and the full report goes to your Home Assistant notifications. MAC and IP addresses and client hostnames are redacted from everything Sentinel and the audit tool show a language model. Optional LLM-powered triage and rule discovery — covering power, battery, and environmental sensors (temperature, humidity, CO₂, air quality, …). Approved discovery rules can be inspected, deactivated, reactivated, and surgically repaired via HA services. |
...
> **The agent only sees entities that are exposed to Assist.** Home Assistant exposes new entities automatically only for a fixed set: lights, switches, covers, fans, climate, media players, scenes, humidifiers, to-do lists, vacuums and water heaters, binary sensors of class door, window, opening, garage door, lock, motion or presence, and sensors of class temperature, humidity, CO, CO₂, AQI, PM2.5, PM10 or VOC. Everything else stays hidden until you expose it — a leak (`moisture`), smoke, gas, vibration or tamper sensor, a lock, a battery level, a power meter — and the agent will tell you it has no such device even though the entity is right there in Home Assistant. After pairing a new device, open **Settings → Voice assistants → Expose** and expose the entities you want to ask about, then give the device the name and area you will use when asking ("Kitchen Sink Flood Sensor" in "Kitchen"): the agent finds entities by their name and area, so a device still called `Shelly Flood S Gen4 A1B2C3` with no area answers to neither "kitchen" nor "sink". Sentinel is not affected — its rules read every entity, exposed or not. See [Control Home Assistant (LLM API)](docs/configuration.md#control-home-assistant-llm-api).
