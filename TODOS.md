...
**What:** `_strip_env_context_id_tokens` (discovery_semantic.py) folds `*_away`/`*_day`/`*_night` tails off environmental candidate IDs, so two candidates with different sensors (distinct semantic keys, both stored) can converge on one `candidate_id`; `discovery_store.find_candidate` returns the newest match and `proposal_store` lookups are ID-keyed, so promoting one card could resolve to the other candidate or report a false existing-rule collision (Codex adversarial, env-context-sanitizer ship 2026-08-15).
