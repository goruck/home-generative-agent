...
**How to apply:** Give each of the three the same shape #559 landed for the analyzer: `@callback` on the listener (so the body runs in the same loop tick as `_OneTimeListener`'s unsubscribe, not on an executor thread), a `fired` flag, and a `@callback` cancel closure wrapped in `entry.async_on_unload`. The repetition across four sites is the argument for a small local helper — something like `_defer_start_until_hass_started(hass, entry, start_fn)` returning nothing — rather than a fourth copy. Extend `tests/custom_components/home_generative_agent/test_deferred_start_listener.py`, which already has the harness: assert unload-before-STARTED cancels each engine's start, and that firing STARTED runs the body on the main thread.
