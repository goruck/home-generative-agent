...
**What is left:** the disabled-image-entity sub-case above (face recognition running, but the user disabled `image.*_last_event` in the entity registry, so `recognized_people` is empty and the rules are inert with the repair issue silent, because the option and the gallery both look healthy).
