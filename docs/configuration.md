...
The tool universe is per-request: Home Assistant exposes some Assist tools only for certain devices (the timer intents — `HassStartTimer` and friends — exist only when the requesting device supports timers), and MCP tools exist only while their server is reachable. The index reconciles with this automatically in both directions. When a turn's device exposes tools the index has never seen, they are discovered and indexed inline before retrieval runs, so the very first "set a timer for two minutes" from a voice satellite binds `HassStartTimer`. Conversely, indexed tools that don't exist for the current turn (timer tools in a browser chat, tools of an MCP server that failed to load) are never bound.
