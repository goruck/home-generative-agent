...
- Activating a scene, calling a script, triggering another automation, firing an event, pressing a button, calling `python_script` / `shell_command` / `rest_command`, or handing text to `conversation.process` (which can dispatch an intent whose `intent_script` unlocks a door): those all run configuration stored elsewhere. Expect a PIN prompt for these even when the target is harmless.
