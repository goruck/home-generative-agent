...
| `RECOMMENDED_SENTINEL_RESPONSE_LANGUAGE` | `sentinel_response_language` | `""` (English) | Optional language override for the LLM-authored finding explanation shown in notifications (`explain/llm_explain.LLMExplainer`). Discovery candidate `title`/`summary` and Sentinel Triage `decision`/`reason_code`/`summary` are unaffected — those are parsed by code downstream and always stay in English (see #524). |
