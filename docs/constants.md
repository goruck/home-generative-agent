...
| `_COOCCURRENT_FACES` | `core/video_analyzer.py` | `2` | Identity entries in one frame that count as two detected face boxes. A frame at or above this refuses both the unknown-face merge and the single-person summary verdict — the no-face sentinel is only ever a lone entry, so two entries always mean two real detections even when one embedding degraded to Indeterminate (issue #543) |
...
| `_COOCCURRENT_FACES` | `2` | Identity entries in one frame that count as co-occurring face detections; refuses both the unknown-face merge and the single-person summary verdict |
