...
**Battery-level sensor filtering:** Battery-level sensors (the charge percentage of a device or phone) are likewise excluded from the monitoring gap list. A battery percentage declines monotonically, so a rolling-average baseline on it is only a laggy low-battery detector — threshold rules are the right tool, and gap-hinting these sensors made the LLM propose occupancy- and night-conditioned battery candidates that the low-battery threshold template never evaluates. Classification is metadata-first: a live `device_class: battery` marks a charge level whatever the entity ID says, any other declared device class or a non-`%` unit marks a telemetry stream, and only when metadata is absent does the entity-ID heuristic decide — a measurement token after the battery token (`sensor.battery_power` on a home battery) is real telemetry that stays baseline-eligible, while device-named battery levels (`sensor.front_door_battery`, `sensor.garage_temperature_sensor_battery`) are excluded. The discovery prompt additionally instructs the model never to condition a battery-level candidate on occupancy or time of day (a notable battery level should surface as a plain low-battery threshold idea), and battery candidates share one context-free dedup key that matches the `low_battery_sensors` rule a promotion registers — so occupancy/night variants of the same battery idea no longer pile up as distinct novel candidates, and an activated rule suppresses re-proposals of it.
