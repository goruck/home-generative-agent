...
| `unavailable_sensors` | Sensors (`sensor.*` or `binary_sensor.*`, e.g. occupancy/presence) in `unavailable` state |
| `unavailable_sensors_while_home` | Sensors (`sensor.*` or `binary_sensor.*`) in `unavailable` state while someone is home |
