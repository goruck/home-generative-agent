...
|---|---|---|
| `ha_sensitive_entity_exposed_without_pin` | high | A lock, alarm panel, or door/gate/garage cover is exposed to Assist, Alexa, or Google Assistant while the [Critical Action PIN](configuration.md#critical-action-pin) is not enforceable. One finding per assistant. |
| `ha_new_admin_or_token` | high / medium | A new administrator account, a new long-lived access token, or a long-lived token used from an address never seen before (medium). Compared against a persistent inventory — see below — so each change is reported exactly once. |
| `ha_long_lived_token_stale` | low | A long-lived access token unused for `sentinel_ha_token_stale_days` (default 90). A token never used counts from its creation date. |
...
| `ha_http_proxy_misconfigured` | medium | IP banning after failed logins is disabled (`ip_ban_enabled: false` or no `login_attempts_threshold`). Home Assistant already refuses `use_x_forwarded_for` without `trusted_proxies`, so that pairing is not re-checked. |
...
- **Auth inventory.** The auth-change rule compares against a small persistent store (`.storage/home_generative_agent_sentinel_auth_inventory`) holding user ids, admin flags, token ids, client names, and HMAC-pseudonymized last-used addresses. It never holds token values or raw IPs. The first run records everything as known and posts one *Sentinel auth inventory established* notification instead of alerting on every pre-existing token; restarts do not re-bootstrap. `home_generative_agent.sentinel_reset_auth_inventory` clears it, and deleting the Sentinel subentry deletes it together with the pseudonymization salt.
...
- **Deterministic copy.** Every finding carries a `summary` sentence with the exact facts (which token, which port, how many minutes), and the notifier renders it as the mobile and persistent text. The summary is display-only and does not affect the anomaly id, so snooze and cooldown state survive a changing count.
...
| `sentinel_ha_token_stale_days` | `ha_long_lived_token_stale`: days a long-lived access token may go unused before it is reported (default 90) |
...
| `home_generative_agent.sentinel_reset_auth_inventory` | Clear the persistent user/token inventory behind `ha_new_admin_or_token`; the next run records the current state as known without alerting |
