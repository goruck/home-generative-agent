...
| `sentinel_response_language` | Optional language (e.g. `Czech`) for the LLM-authored finding explanation shown in mobile and persistent notifications (the "Enable LLM explanations for sentinel findings" feature). Empty (default) leaves the model's native English output. Recognized person names are always kept in their base (nominative) form so [`_redact_if_sensitive`](../custom_components/home_generative_agent/sentinel/notifier.py)'s exact-match redaction still works. Discovery candidate `title`/`summary` and Sentinel Triage `decision`/`reason_code`/`summary` are **not** affected by this setting — those fields are parsed by code downstream (normalization, dedup, evidence matching) and always stay in English; see [#524](https://github.com/goruck/home-generative-agent/issues/524) for the tracked follow-up to make that translatable safely. Mirrors `vlm_response_language`'s convention (see the VLM options docs). |
