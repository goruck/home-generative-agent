...
| `sentinel_response_language` | Optional language (e.g. `Czech`) for the LLM-authored finding explanation shown in mobile and persistent notifications (the "Enable LLM explanations for sentinel findings" feature). Empty (default) leaves the model's native English output. For sensitive findings, recognized person names are removed from the model's input before the explanation is generated, so no inflected form can bypass [`_redact_if_sensitive`](../custom_components/home_generative_agent/sentinel/notifier.py)'s exact-match redaction; the prompt additionally instructs the model to keep any names in their base (nominative) form. Discovery candidate `title`/`summary` and Sentinel Triage `decision`/`reason_code`/`summary` are **not** affected by this setting — those fields still stay in English; translating them remains a future follow-up (the structured-evidence groundwork that unblocks it landed via [#524](https://github.com/goruck/home-generative-agent/issues/524)). Away/home/night context for discovery candidates is resolved primarily from structured evidence paths (`derived.anyone_home`, `not derived.anyone_home`, `derived.is_night`) rather than English wording, so normalization, dedup, and the proposals-card preview no longer depend on English prose for that context. Mirrors `vlm_response_language`'s convention (see the VLM options docs). |
