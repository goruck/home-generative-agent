...
| `sentinel_response_language` | Optional language (e.g. `Czech`) for the LLM-authored finding explanation shown in mobile and persistent notifications (the "Enable LLM explanations for sentinel findings" feature). Empty (default) leaves the model's native English output. For sensitive findings, recognized person names are removed from the model's input before the explanation is generated, so no inflected form can bypass [`_redact_if_sensitive`](../custom_components/home_generative_agent/sentinel/notifier.py)'s exact-match redaction; the prompt additionally instructs the model to keep any names in their base (nominative) form. Several finding types (`alarm_disarmed_during_external_threat`, `appliance_power_duration`, `alarm_disarmed_open_entry`, `baseline_deviation`, `time_of_day_anomaly`, `entity_staleness`) normally render their mobile push from a deterministic English formatter that quotes exact figures and entity names; when a response language is set, those defer to the translated explanation so the mobile push matches the persistent notification, and fall back to the English formatter only when no usable explanation exists (explainer disabled, or a translation over the 220-character mobile cap). Three surfaces are still English in all cases, because they are fixed strings assembled from counts and finding-type labels rather than model output: the notification **title** (`Security Alert` / `Home Alert` / `Home Update`) and **subtitle** line, the **burst-batch summary** (when more than three non-high-severity notifications fire within 60 seconds they collapse into a single `"N home updates: …"` push and the translated per-finding text is not delivered), and the **daily digest**. Discovery candidate `title`/`summary` and Sentinel Triage `decision`/`reason_code`/`summary` are **not** affected by this setting — those fields still stay in English; translating them remains a future follow-up (the structured-evidence groundwork that unblocks it landed via [#524](https://github.com/goruck/home-generative-agent/issues/524)). Away/home/night context for discovery candidates is resolved primarily from structured evidence paths (`derived.anyone_home`, `not derived.anyone_home`, `derived.is_night`) rather than English wording, so normalization, dedup, and the proposals-card preview no longer depend on English prose for that context. Mirrors `vlm_response_language`'s convention (see the VLM options docs). |
