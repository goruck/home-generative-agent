...
These rules audit Home Assistant's own attack surface and need no router or third-party integration — every fact comes from Home Assistant's auth store, registries, config flows, and runtime settings. They are the first phase of the [network security and privacy audit plan](network-security-plan.md); router, DNS, and radio-protocol adapters (unknown devices, UPnP, guest networks, Z-Wave security classes, Zigbee permit-join) follow in later phases and plug into the same snapshot section. Nothing here scans the network.
...
- **One alert per day for standing conditions.** A stale token or an exposed add-on port does not change from one cycle to the next, so these rules carry a 24-hour cooldown floor (`SENTINEL_POSTURE_RULE_COOLDOWN_MINUTES`) that overrides a shorter `sentinel_cooldown_minutes`. `security_device_unavailable` and `ha_new_admin_or_token` are event-like and use the normal cooldowns.
