...
|---|---|---|
| `ha_sensitive_entity_exposed_without_pin` | high | A lock, alarm panel, or door/gate/garage cover is exposed to a voice assistant. For Assist the finding is suppressed while the [Critical Action PIN](configuration.md#critical-action-pin) is enforceable; exposure to Alexa or Google Assistant is always reported, because those reach Home Assistant through the cloud integration and never see the PIN. Default exposure counts (a cover Assist exposes by default is exposed). |
...
| `ha_addon_exposed_port` | medium / high | A running Supervisor add-on maps a container port onto the host. High for shell, file-share, and database add-ons (SSH, Samba, MariaDB, PostgreSQL, InfluxDB, …). HA OS and Supervised only. |
