...
- **Every entry must contain a dot and at least one literal character** (entity IDs are `domain.object`). Match-everything spellings such as `"*"` or `"*.*"` are rejected by the settings form and ignored (with a log warning) if hand-edited into storage — for a security engine, one typo'd pattern must not silently disable all monitoring. To exclude an entity from every rule, put it under the `"*"` *type key* instead.
