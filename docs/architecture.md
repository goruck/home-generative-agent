...
Phase 1 ships the `ha_native` adapter (Home Assistant's own attack surface); `snapshot/radio.py` adds the Zigbee, Z-Wave, Bluetooth, and Matter device adapters, `snapshot/upnp.py` the UPnP/IGD adapter (the SSDP discovery cache and the core `upnp` integration's sensors), and `snapshot/router.py` the router client adapters: `generic_router_tracker` turns every `device_tracker` with `source_type: router` into a pseudonymized client and asks the device inventory which are new, `eero` reads the eero integration's network-level switches and sensors as router posture, and `snapshot/eero.py` is the eero runtime adapter that reads the integration's coordinator (its client list and settings) so a client is seen without a device tracker. The remaining router and DNS adapters are described in the [network security plan](network-security-plan.md). Identifiers that could single out a device or person (token last-used addresses today; MACs and Bluetooth addresses in later phases) are pseudonymized with a per-install HMAC salt (`sentinel/pseudonymizer.py`) before anything persists them, and `sentinel/redaction.py` is the matching gate on the way out: every structure about to be rendered into a prompt (a finding's evidence for the explainer, the triage prompt, the reduced discovery snapshot, the `audit_home_security` tool payload) has address-bearing keys dropped, hostnames replaced by manufacturer plus key, and embedded MAC/IP addresses replaced by class tokens.
