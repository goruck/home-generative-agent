...
| `add_automation` | Create and register a HA automation (available when Schema-first YAML mode is disabled). An automation whose actions reach a critical service is held for PIN confirmation before it is written — see [Critical Action PIN](configuration.md#critical-action-pin). Before either, every concrete entity ID and service name in the validated config is checked against Home Assistant (`agent/automation_targets.py`): the Assist API shows the model names, never entity IDs, so a trigger on `binary_sensor.sink_moisture_sensor` derived from "Sink Moisture Sensor" or an action calling the remembered `notify.mobile_app` passes HA's schema validation and installs as an automation that never runs. The tool refuses such a config and returns each missing name with the closest real one (the entity whose friendly name slugifies to the guess, the configured mobile push service for a missing `notify.*`, a domain's real services otherwise), so the model's retry is deterministic. Templates, registry IDs, areas, `entity_id: all` and device actions are left to Home Assistant |
