...
**Snapshot disk retention.** The analyzer keeps a rolling window of the most recent 200 snapshot files per camera (`VIDEO_ANALYZER_SNAPSHOTS_TO_KEEP`) and deletes the oldest as new ones arrive. Files are registered for retention the moment they are captured, so frames dropped later in the pipeline (VLM errors, summary timeouts, backlog drops) cannot accumulate on disk. On startup, snapshots left over from before a restart are folded into the same retention window as the oldest entries — only files the integration itself wrote (`snapshot_YYYYMMDD_HHMMSS.jpg`) are claimed, so a photo you place in a camera's snapshot folder yourself is never touched. The published `latest.jpg` and images attached to notifications from the last 30 minutes are protected from cleanup.
