...
| Step | Scope | State |
|---|---|---|
| 1–5 | HA-only MVP: `platform` on snapshot entities, the `ha_native` adapter, the auth inventory, capability gating, the Home Assistant rules | Shipped in v3.39.0 (PR #613); field-validated 2026-09-08 |
| 6 | Radio adapters, the device inventory, the five radio rules | Shipped in v3.42.0 (PR #634); design in the [radio plan](network-security-radio-plan.md) |
| 7 | Router adapters and client rules | Shipped: `upnp_igd` (PR #637), `generic_router_tracker` and eero posture with `network_unknown_device_joined` (PR #641), the eero runtime adapter (PR #643), the four eero posture rules (PR #645), `network_guest_client_present` (PR #646), the single-device Trust gate (PR #664). Open: `fritz`, `unifi`, `adguard` (no hardware to validate on; each needs a pre-release volunteer), `network_port_forward_active`, `network_iot_device_on_main_vlan` |
| 8 | Redaction gate; `network_audit` feature type | Redaction shipped (PR #640). The feature type is dropped until Sentinel has a second consumer for a pinned audit model (see the history) |
| 9 | `audit_home_security` tool, `run_network_audit` service, privacy digest | Tool and service shipped in v3.40.0 (PR #627); the deterministic digest replaced the local-model override (PR #649) |
| 10 | Baseline counters, discovery templates | Shipped: per-client usage baselines and `network_client_usage_anomaly` (PR #665), the three network templates (PR #666) |
| 11 | Docs and CHANGELOG | Shipped (PR #667) |

The history below records every deviation from the design and every review round, in order.

Phase 1 (HA-only MVP, implementation-order steps 1–5) shipped in PR #613 as v3.39.0 and field-validated on 2026-09-08: the five findings it raised on a real install were all judged worth receiving, which was this plan's gate for the later steps. Two review rounds refined the design. The multi-agent review made every rule in this family emit one aggregated finding per cycle (the per-type cooldown would otherwise starve sibling findings), commit the auth inventory after dispatch with hold-back of undelivered changes, and turn the token rule into an age rule: Home Assistant records refresh-token use only on the access-token exchange, which a long-lived token performs once at creation, so `long_lived_tokens_unused_days` became `long_lived_token_age_days` and `refresh_tokens_from_new_ip` is collected but not alerted on. The Codex adversarial round then added `assist_agents_outside_pin` (the Critical Action PIN silences Assist exposure only when every pipeline runs on this integration's agent), made add-ons with unfetched Supervisor details a named gap instead of a silent pass, made the inventory retry failed writes before announcing itself, and dropped the derived `http_use_x_forwarded_for` field because core never stores it. Step 6 (radio adapters, the device inventory, and the five radio rules) is implemented as described in `docs/network-security-radio-plan.md`, with three recorded deviations: ZHA permit-join is reported as not observable on Home Assistant 2026.9 instead of read, the inventory bootstraps per protocol instead of once, and `zwave_inclusion_active` is a rule of its own. Step 7 has started with the `upnp_igd` adapter (`snapshot/upnp.py`), chosen first because it needs no router integration and was the only adapter validatable on the maintainer's install: `network_upnp_enabled` reads the SSDP discovery cache (an IGD advertisement is the proof), a loaded `upnp` entry with live entities, or a pending `upnp` discovery flow, and reports the capability as missing rather than asserting UPnP off when none is seen; `network_public_ip_changed` (pulled forward from phase 2) and a new `network_upnp_port_mapping_added` rule compare the integration's external-IP and port-mapping-count sensors against the previous run's values, remembered in the device inventory store together with the sensor they came from. The review round added a bounded SSDP read (the pinned description cache can leave waiters hanging), rejection of non-finite counts and of the ``0.0.0.0`` WAN-down address, exclusion of ignored config entries, a pending discovery flow trusted only when the cache is unreadable, and Markdown escaping of network summaries in persistent notifications. WAN status is not read: no rule consumes it. Step 8's redaction half shipped next, ahead of the remaining step 7 adapters because every one of them fills the client list with MACs, IPs, and hostnames: `sentinel/redaction.py` (`redact_network_identifiers`) is applied at the four model boundaries (a finding's evidence and rendered prompt in the explainer, the triage prompt, the reduced discovery snapshot and rendered prompt, the `audit_home_security` tool report before clipping) and drops address-bearing keys, replaces a client's hostname with manufacturer plus key, and tokenizes addresses embedded in free text (separated or bare MACs, IPs by subnet class, local-suffix hostnames) to `[mac]` / `[lan ip]` / `[public ip]` / `[hostname]`; the notifier and the service response keep the real values, and LAN-advertised names without an address (discovery titles, gateway names) still reach the model as labelled data per the tool's earlier decision. The review round (Codex + Claude) fixed MAC boundaries at punctuation, bare-hex MACs in mDNS names, IPv6 false positives, the unwalked hostname replacement, tuple subclasses, dict-key collisions, unbounded traversal, and the ungated prompt parts (suggested actions, semantic keys). The `network_audit` feature type (the local-model override) is deferred to the step 9 remainder, where the digest and in-tool narrative give it a consumer: today the explainer skips the security family, triage admits no identifier, and discovery drops the section, so the subentry would change nothing, which the release policy treats as a defect rather than a feature. When it lands, note that `FEATURE_CATEGORY_MAP` is one-to-one with the model categories and the resolver assumes it: mapping `network_audit` onto `chat` must first make `conversation` the owner of that category, or the audit pin would silently move the chat model. Step 7 continued with the two adapters validatable on the maintainer's network, `generic_router_tracker` and `eero` (`snapshot/router.py`), and the first client rule, `network_unknown_device_joined`. Deviations from the text above: the raw MAC never enters the snapshot (the client carries its pseudonymized `key` and a `mac_randomized` flag derived in the adapter; nothing downstream needed the address); eero's per-client sensors (signal, usage, blocked) wait for the baseline step since no rule reads them; the generic adapter handles eero's trackers too (they are ordinary scanner entities), so the eero adapter contributes posture and counters only and the merge dedupes clients by key with the later adapter winning; the inventory reconciles only the sources a run observed, so a radio-only commit never deletes router rows; the grace period is applied by the rule from the inventory row's `first_seen` (the client is recorded on first sight, alert owed, and reported once the row is old enough, connected or not: the first field test lost a visitor that stayed two hours and left two minutes before eero listed it, so an offline client is named "not connected now" instead of withheld); auto-trust is decided in the adapter from a device-registry MAC index (a registry device with a config entry from a non-router, non-discovery domain that owns a non-tracker entity) and recorded by the inventory as trusted without an alert; the randomized-MAC suppression is a low-severity report rather than a skip; and the trust services accept inventory keys (`device_key`) because most clients have no registry device. `sentinel_network_router_platform` is not implemented: with one generic adapter and dedupe by key there is nothing to force. The review round (Codex 11 findings, Claude 10, converging) then made auto-trust positive rather than denylisted (a vouching config entry's domain must provide no `device_tracker` anywhere in the home, so an unlisted router cannot vouch for its own clients), settled only the client keys a delivered finding actually named (the rule withholds clients inside the grace period), kept router rows for 30 days after their last sighting instead of deleting them on absence (one router integration reloading shares the `router` source with the others), applied a late auto-trust to a pending row, tokenized tracker names that are addresses, took the DHCP hostname out of the summary (the tracker name or manufacturer plus key instead), made the public-IP posture group atomic in the merge (eero's current key next to the IGD adapter's change flag would have reported a change against the wrong sensor), judged several eero networks together, withheld clients when the registry read failed, fixed the exclusion callback's argument order, and named eero as UPnP evidence in the rule's copy. Field validation of the client rule then showed the tracker path's limit in practice: a visitor that joined at 21:09Z and stayed 2 h 20 min was never reported because eero had created no tracker, and the rule's connected gate withheld the second candidate; the gate was dropped (an offline client is reported once, marked "not connected now") and the eero runtime adapter was added so clients are read from the integration's own data. The four router posture rules then shipped over the settings the eero read supplies (`sentinel/rules/network_router_posture.py`): `network_guest_network_idle` (with a guest-last-active time kept in the engine's posture memory and derived once per run after the merge, `derive_guest_idle`), `network_wpa3_disabled`, `network_protection_disabled` (malware blocking only; ad blocking off is mentioned, not a finding), and `network_ddns_enabled`. The eero Plus-only settings are judged per network where Plus is active and withdrawn from the merge when Plus is known off everywhere, so a non-subscriber (or a lapsed one with leftover switches) is never told to turn on a paid feature. The review round (Codex 7, Claude 10, converging) made the guest clock advance only on a fresh same-source reading, hold across short gaps and restart after two unwatched days, reject future timestamps, and store days rather than instants so the memory is written at most daily; a stale eero poll publishes no settings; the runtime tier names the deciding network's switch as the entity twin so exclusions and the finding's identity match the entity tier; and the rules lean on the engine's exclusion filter instead of their own. `network_guest_client_present` followed, with three deviations from the table below: it reports only clients the device inventory holds as not trusted (devices parked on the guest network on purpose are bootstrap-trusted, and a visitor is one Trust tap away from silence), night is not a trigger (the derived flag follows the sun, so it fired on a winter dinner guest; and a low night alert would have held the per-type cooldown against the medium away alert for a day), and a client's first day belongs to `network_unknown_device_joined` so one arrival never pushes twice. "Nobody is home" requires at least one tracked person, since `anyone_home` is false on an install with no person entities. Its capability, `network.clients.is_guest`, is decided after the merge rather than asserted by an adapter: one source reporting the flag is enough, connected clients without it are a counted gap in the notes, and the flag is never back-filled from an older source (a tracker attribute from the last reload must not describe the runtime read's connection). The review round (Claude 10 findings; Codex stopped on its usage limit after 4 interim findings, all among the 10) also skipped auto-trusted and known rotated-address clients, stopped deferring to an unknown-device alert that never settles, and moved the exclusion and row-age checks the two client rules share into `network_common`. A full Codex pass on the fixed branch (5 findings) then made "nobody is home" positive (`nobody_home_for_sure`: a person with trackers and no readable state means "not sure", since the derived context counts `unknown` as away), turned the trusted-name match from a skip back into a marked hint (the name is self-reported, so a skip was a way to hide), and offered the Trust button only for a single-device finding (one tap trusts every device the finding carries, and the push shows 220 characters); the per-type daily floor starving a second guest and sub-daily MAC rotation outrunning the first-day holdoff are accepted limits filed in TODOS.md. Step 10's baseline half shipped next: the eero runtime read now carries each client's rates, signal, today's traffic, and blocked count (the traffic and blocked figures need the integration's Activity option for clients), the runtime adapter publishes them as `network.client.<key>.<figure>` counters, the engine hands the section's counters to the baseline updater every cycle (`async_record_counters`, kept to the updater's interval, no per-counter notices, left out of the discovery-ready list) and injects their statistics back as `counter_baselines` with the `network.counter_baselines` capability, and `network_client_usage_anomaly` reports a connected device whose upload (medium) or download (low) so far today is 300% and at least 250 MB above its hourly baseline. Deviations from the text below: eero exposes today's cumulative bytes rather than a per-cycle delta, so the hourly profile of the cumulative figure is compared directly ("what this device had usually moved by this hour"), the network-level counters (`network.client_count`, `network.threats_day`) ride along but no rule reads them yet, and the finding is a static rule of the network family rather than a `baseline_deviation` dynamic rule, so it needs no proposal. The review round (Codex 7, Claude 9, converging) then moved the counter write off the detection path (the engine offers, the updater's own loop writes; the fetch is bounded to a timeout and to the two hour buckets the rule reads), split the finding per device and direction with a one-day cooldown on a per-device pseudo entity (a new `entity_cooldown_minutes` rule floor) so one device's alert never hides another's, read "usual" as the larger of this hour and the next (a transfer that always lands late in the hour was never in its own bucket), granted the capability only when a reporting device is covered and noted the rest, stored only the two traffic figures with no weekly rows, expired counter rows after 30 days, left counters out of the health counts, and let the reset service reach them. The discovery half of step 10 followed: the discovery reducer carries a pseudonymized `network` section (client key, display name, connected, guest flag, `trusted` when false; the boolean posture settings), the prompt names the three patterns and the paths to cite, and `network_client_present_when`, `network_client_absent_when`, and `network_posture_equals` are supported templates with evaluators in `dynamic_rules.py`. The path and prose readers (`network_client_keys`, `network_posture_keys`, `network_absent_signal`, `network_posture_expected`) live in `sentinel/evidence_paths.py` so the normalizer and both semantic keys resolve a candidate the same way and an activated rule dedups its own re-proposals. Deviations from the text below: the client templates address a device by its pseudonymized key rather than an entity, the absent template has no time-of-day window beyond the night flag (a candidate can cite `derived.is_night`), and network findings from templates render through the deterministic network copy rather than the explainer. The remaining step 7 adapters (FRITZ!Box, UniFi, AdGuard, none validatable here), `network_port_forward_active` and `network_iot_device_on_main_vlan` (need routers that expose them) are what remains; see TODOS.md under *Sentinel Network Audit*. Implementation-order step 9 shipped early in its HA-only form (v3.40.0): `audit_home_security` and `run_network_audit` return the findings of the phase 1 rules plus the checks that could not run and why. The step 9 remainder then shipped as a deterministic digest instead of the local-model override described under *Local-model override* below: with `sentinel_network_audit_share_details` off, `audit_home_security` returns only counts, severities, rule ids, fixed per-rule titles, the checks that ran, and the static reasons others could not, and, when a signed-in administrator asked, posts the full report as a persistent notification (a voice satellite or a non-admin account gets the digest only, so a guest cannot publish or overwrite it). The review round (Codex 3 findings, Claude 9, converging) added that administrator gate, made the privacy switch fail closed, gave the digest a count of coverage notes so "no findings" is never said over checks that ran on incomplete data, and recorded the limit that a conversation which already holds audit details keeps sending them until a new one is started. The override was dropped for two reasons found when it came to be built. Its narrative, written by the audit model from the full payload, was itself handed to the conversation model, so the promise that no names leave the tool rested on how an LLM happened to phrase things. And nothing else would have used the pinned model (the explainer skips the security family, triage admits no identifier, discovery drops the section), while the model plumbing is per category, so a `network_audit` feature needed either a fifth model category or a second owner of `chat`. The `network_audit` feature type is therefore not planned until Sentinel has a real second consumer for it (network triage or discovery in step 10). Step 11, the docs pass (PR #667), found the rules table, services, template group, and CHANGELOG already current from the earlier steps and added what was missing: help text under each of the seven security options in the Sentinel flow (English and Czech), the architecture guide's network section rewritten around the shipped path (adapters and the eero runtime tier, both inventories, counter baselines, discovery, the on-demand audit, the privacy gate), the remaining code-only constants in the constants reference, and the status table above.
...
| `generic_router_tracker` | any `device_tracker` with `source_type: router` | `clients[]` with the pseudonymized key, ip, hostname, and the extras a tracker exposes (shipped: `snapshot/router.py`) | 1 |
...
| `eero` | `platform == "eero"` | entity tier (`snapshot/router.py`): the network-level switches and sensors; runtime tier (`snapshot/eero.py`): the coordinator's client list and settings, which win the merge | 1 |
...
has no tracker until the integration is reloaded. The eero *runtime*
adapter (`snapshot/eero.py`) therefore reads the coordinator the integration
keeps at `hass.data["eero"][<entry id>]["coordinator"]` (its `data.networks`,
...
> **Superseded.** Not implemented; see the status paragraph at the top. The shipped mechanism is the deterministic digest (`sentinel_network_audit_share_details`), which needs no audit model and gives a guarantee this design could not. The text below is kept for the reasoning.
...
All done as of step 11 (PR #667); each earlier step kept its own docs current and the pass filled the gaps.

- README: feature row and the "Ask whether the home is secure" example.
- `docs/sentinel.md`: rules table rows, capability gating, inventory services, the eero setup notes, network counters under Baseline Collection, the *Network* template group.
- `docs/architecture.md`: snapshot `network` section and adapter diagram.
- `docs/configuration.md`, `docs/constants.md`: new keys, plus `data_description` help text under each option in the Sentinel flow.
- CHANGELOG entry per phase (under Unreleased until the monthly bundle).
